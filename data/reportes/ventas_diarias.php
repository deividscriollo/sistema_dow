<?php
    ...
    {   
        ...
        function Header(){             
            ...
            while($row = pg_fetch_row($sql)){
                ...
                $this->Text(75,49,"VENTAS DIARIAS",0,'C',0);
                $this->Text(75,49,"VENTAS DIARIAS",0,'C',0);                                                                
            }            
    ...
    $pdf->Cell(65, 5, "NOMBRE",1,0, 'C',0);
    $pdf->Cell(25, 5, "CANTIDAD",1,0, 'C',0);    
    $pdf->Cell(25, 5, "PRECIO",1,0, 'C',0);        
    $pdf->Cell(25, 5, utf8_decode("TOTAL"),1,1, 'C',0);    
    ...
    $sql = "select productos.codigo, productos.descripcion,sum(cantidad::float) as tot_cant, sum(detalle_factura_venta.precio::float) as tot_precio, sum(detalle_factura_venta.total::float) as tot_total from detalle_factura_venta,productos where detalle_factura_venta.id_productos = productos.id_productos and detalle_factura_venta.fecha_creacion between  '".$inicio."' and '".$fin."' group by detalle_factura_venta.id_productos, codigo ,descripcion ";
    $sql = sql($conexion,$sql);      
    while($row = pg_fetch_row($sql)){                
        $pdf->Cell(60, 5, maxCaracter(utf8_decode($row[0]),13),0,0, 'L',0);
        $pdf->Cell(65, 5, maxCaracter(utf8_decode($row[1]),30),0,0, 'L',0);
        $pdf->Cell(25, 5, maxCaracter(utf8_decode($row[2]),15),0,0, 'C',0);        
